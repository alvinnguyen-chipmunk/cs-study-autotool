	...
	if [ "$OS_TYPE" = "win" ]; then
		...
		if [ $IS_MAKE -eq $TRUE ]; then
                        if [ "$IS_RELEASE" = "NA" ]; then
                            FOLDER=${HERE}/build_win_debug
                        else
                            FOLDER=${HERE}/build_win_release
                        fi
		elif [ $IS_PACKAGE -eq $TRUE ]; then
			if [ "$IS_RELEASE" = "NA" ]; then
                            FOLDER=${HERE}/build_win_package_debug
                        else
                            FOLDER=${HERE}/build_win_package_release
                        fi	
		fi

		if [ "$DIR_INSTALL" = "NA" ]; then 
			DIR_INSTALL="$FOLDER/package/AutotoolDemo"
		fi
...
package_stylAutotool()
{
	...
	if [ "$OS_TYPE" = "win" ]; then
		...
		make && make install && \
		echo cp -rf $(pwd)/../package/nsis_data/depend-package-win/* $DIR_INSTALL/.. && \
                cp -rf $(pwd)/../package/depend-package-win/* $DIR_INSTALL && \
		echo cp -rf $(pwd)/../package/nsis_data/* $DIR_INSTALL/.. && \
		cp -rf $(pwd)/../package/nsis_data/* $DIR_INSTALL/.. && \
		echo tar zxvf $(pwd)/../package/EnvVarUpdate.tar.gz -C $DIR_INSTALL/.. && \
		tar zxvf $(pwd)/../package/EnvVarUpdate.tar.gz -C $DIR_INSTALL/.. && \
		cd $DIR_INSTALL/.. && \
		makensis setup.nsi 
		cd -
	else
